...
export default function Entrainement() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const { data: p } = await supabase.from('profil_joueur').select('*').eq('joueur_id', joueurId).single()
      const { data: t } = await supabase.from('tirs').select('*').eq('joueur_id', joueurId).order('date', { ascending: false })
      const { data: r } = await supabase.from('recommandations').select('*').eq('joueur_id', joueurId)
      const { data: o } = await supabase.from('objectifs_seance').select('*').eq('joueur_id', joueurId)
      const { data: d } = await supabase.from('stats_dribble').select('*').eq('joueur_id', joueurId).order('date', { ascending: false })
      const { data: def } = await supabase.from('stats_defense').select('*').eq('joueur_id', joueurId).order('date', { ascending: false })
      setProfil(p)
      setTirs(t || [])
    ...
    <div style={{ maxWidth: 1000, margin: '0 auto', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ background: 'white', borderRadius: 12, padding: 24, border: '1px solid #e5e7eb', marginBottom: 24 }}>
          ...
          {ZONES.map(zone => {
            ...
            return (
              <g key={zone.id} onClick={() => setSelectedZone(isSelected ? null : zone.id)} style={{ cursor: 'pointer' }}>
                <circle cx={zone.cx} cy={zone.cy} r="30" fill={getColor(pct)} stroke={isSelected ? '#2563eb' : getBorder(pct)} strokeWidth={isSelected ? 2.5 : 1.5}/>
                <text x={zone.cx} y={zone.cy - 8} textAnchor="middle" fontSize="8.5" fill="#374151" fontWeight="600" style={{ pointerEvents: 'none' }}>{zone.label}</text>
                <text x={zone.cx} y={zone.cy + 10} textAnchor="middle" fontSize="14" fill="#111" fontWeight="800" style={{ pointerEvents: 'none' }}>{pct !== null ? `${pct}%` : '—'}</text>
              </g>
            )
          })}
        </svg>
        ...
        <div style={{ display: 'flex', gap: 20, marginBottom: 24, fontSize: 12, color: '#6b7280' }}>
          <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><span style={{ width: 14, height: 14, background: 'rgba(34,197,94,0.25)', border: '1px solid #16a34a', borderRadius: 3, display: 'inline-block' }}></span>≥ 60%</span>
          <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><span style={{ width: 14, height: 14, background: 'rgba(234,179,8,0.25)', border: '1px solid #ca8a04', borderRadius: 3, display: 'inline-block' }}></span>40–59%</span>
          <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><span style={{ width: 14, height: 14, background: 'rgba(239,68,68,0.25)', border: '1px solid #dc2626', borderRadius: 3, display: 'inline-block' }}></span>&lt; 40%</span>
        </div>
        ...
        {selectedZone && (
          <div style={{ background: '#f9fafb', borderRadius: 10, padding: 16, border: '1px solid #e5e7eb', marginBottom: 24 }}>
            <h3 style={{ fontSize: 14, fontWeight: 700, marginBottom: 12 }}>Détail — {ZONES.find(z => z.id === selectedZone)?.label}</h3>
        ...
        <p style={{ fontSize: 12, fontWeight: 600, color: '#6b7280', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 12 }}>Gestion du ballon</p>
